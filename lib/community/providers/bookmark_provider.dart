...
class BookmarkProvider with ChangeNotifier {
  ...
  String _getUserBookmarksKey(String userId) => 'user_bookmarks_$userId';
  ...
  Future<void> fetchBookmarks() async {
    try {
      ...
      final userBookmarksKey = _getUserBookmarksKey(userId);
      ...
      // Fetch local bookmarks as backup - now using user-specific key
      final prefs = await SharedPreferences.getInstance();
      final String? storedBookmarks = prefs.getString(userBookmarksKey);
      ...
      // Save merged bookmarks back to local storage with user-specific key
      _saveBookmarksLocally(userId);
    } catch (error) {
      _isLoading = false;
  ...
  // Save bookmarks to local storage using user-specific key
  Future<void> _saveBookmarksLocally(String userId) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      ...
      final userBookmarksKey = _getUserBookmarksKey(userId);
      await prefs.setString(userBookmarksKey, jsonData);
    } catch (error) {
