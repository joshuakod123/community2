...
class _PostDetailScreenState extends State<PostDetailScreen> {
  ...
  Future<void> _fetchComments() async {
    ...
    try {
      // Handle the case when id might be a string or an int
      final postId = widget.posts.id;
  ...
  Future<void> _submitComment() async {
    ...
    final newComment = Comment(
      ...
      userId: supabase.auth.currentUser?.email ?? supabase.auth.currentUser?.id,
    );
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('Failed to add comment. Error: ${error.toString()}')),
      );
    } finally {
      setState(() {
        _isLoading = false;
  ...
  Future<void> likePost() async {
    ...
    try {
      print("Attempting to like post ID: ${widget.posts.id}");

      // Insert like - handle any type conversion explicitly
  ...
  Future<void> _extendExpiration() async {
    ...
    try {
      ...
      if (userId == null || userId != _postCreator) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Only the post creator can extend the expiration')),
        );
        setState(() {
          _isLoading = false;
        });
      ...
      final currentExpiration = widget.posts.expirationDate ??
  ...
  Future<void> _deletePost() async {
    ...
    try {
      ...
      await Provider.of<Posts>(context, listen: false).deletePost(
          widget.posts.id, context: context);
  ...
  Future<void> _deleteComment(int? commentId) async {
    ...
    try {
      await Provider.of<Comments>(context, listen: false).deleteComment(
          commentId);
  ...
  Widget build(BuildContext context) {
    ...
    String? _user = supabase.auth.currentUser?.email ??
        supabase.auth.currentUser?.id;

    // Check if current user is the post creator
    final bool isCurrentUserPostCreator = _postCreator == _user ||
        widget.posts.userId == _user;
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      Text(
                        widget.posts.datetime != null
                            ...
                            widget.posts.datetime!)
                            : 'No date available',
                        style: TextStyle(color: Colors.grey[600]),
                      ),
                      const SizedBox(width: 10),
                      Text('Posted by: ${widget.posts.userId ?? "Unknown"}',
                          style: TextStyle(color: Colors.grey[600])),
                    ],
                  ),
                    ...
                    Padding(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: daysRemaining <= 1 ? Colors.red.withOpacity(
                              0.1) : Colors.amber.withOpacity(0.1),
                          borderRadius: BorderRadius.circular(16),
                          border: Border.all(
                            color: daysRemaining <= 1 ? Colors.red : Colors
                                .amber,
                            width: 1,
                          ),
                        ),
                            ...
                            if (isCurrentUserPostCreator && daysRemaining < 3)
                              Padding(
                                ...
                                child: TextButton(
                                  ...
                                  child: const Text(
                                      'Extend', style: TextStyle(fontSize: 12)),
                                ),
                              ),
                          ],
                        ),
                  ...
                  // Image (optional)
                  if (widget.posts.imgURL != null &&
                      widget.posts.imgURL!.isNotEmpty)
                    Image.network(
                      widget.posts.imgURL!,
                                  ...
                                  if (isCommentOwner)
                                    IconButton(
                                      icon: const Icon(
                                          ...
                                          size: 20),
                                      onPressed: () =>
                                          _showCommentDeleteConfirmation(
                                              comment.id),
                                    ),
                                ],
                              ),
