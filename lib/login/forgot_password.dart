...
class _ForgotPasswordPageState extends State<ForgotPasswordPage> {
  ...
  Future<void> _resetPassword() async {
    ...
    try {
      ...
      if (userResponse == null) {
        // User doesn't exist in users table, check if they exist in auth
        try {
          // Try to send a password reset email through Supabase Auth
          await _supabase.auth.resetPasswordForEmail(email);

          // If successful, the user exists in auth but not in our users table
          // We should handle this case properly
          await _handleAuthOnlyUser(email, newPassword);
        } catch (authError) {
        ...
        // User exists in users table, proceed with custom temporary password
  ...
  // Handle user that exists in auth but not in users table
  Future<void> _handleAuthOnlyUser(String email, String newPassword) async {
    try {
      ...
      await _setTemporaryPassword(email, newPassword, response['id']);
    } catch (error) {
