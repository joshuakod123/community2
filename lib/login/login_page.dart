...
// import '../pages/main_page_ui.dart';  // 실제 경로에 맞게 수정하세요
// import '../pages/profile_page.dart';   // 실제 경로에 맞게 수정하세요
...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<bool> _handleTemporaryPasswordLogin(String email, String password) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      ...
      if (storedTempPassword != null && password == storedTempPassword && storedUserId != null) {
        ...
        if (userData != null && userData['temp_password'] == password) {
          ...
          // Store user session info manually (since we're not using Supabase Auth for temp passwords)
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (await _isTemporaryPasswordAttempt(email)) {
        final tempLoginSuccess = await _handleTemporaryPasswordLogin(email, password);
        ...
        if (tempLoginSuccess) {
          ...
          if (context.mounted) {
            // TODO: 실제 ProfilePage 경로에 맞게 수정하세요
          ...
          _hideLoadingAndShowError("Invalid temporary password or password has expired");
          return;
        }
      }
      ...
      if (response.user != null) {
        ...
        if (context.mounted) {
          // TODO: 실제 MainPage 경로에 맞게 수정하세요
          Navigator.pushReplacementNamed(context, '/main');
          // Navigator.pushReplacement(
          //   context,
          //   MaterialPageRoute(builder: (context) => MainPageUI()),
          // );
        }
      }
