...
class _RegisterPageState extends State<RegisterPage> {
  ...
  Future<void> _signUp() async {
    ...
    try {
      ...
      if (user != null) {
        print("✅ User created in Supabase Auth: ${user.id}");
        ...
        // CRITICAL: Ensure the user's details are in our users table
        // Try multiple times if necessary
        bool userRecordCreated = false;
        int attempts = 0;
        const maxAttempts = 3;

        while (!userRecordCreated && attempts < maxAttempts) {
          attempts++;
          try {
            print("🔄 Attempt $attempts: Creating user record in users table...");

            await _supabase
                .from('users')
                .insert({
              'id': user.id, // Use the actual Supabase Auth user ID
              'email': email,
              'first_name': firstName,
              'last_name': lastName,
            ...
            print("❌ Attempt $attempts failed to create user record: $dbError");

            if (attempts < maxAttempts) {
              // Wait a bit before retrying
              await Future.delayed(Duration(seconds: attempts));
            } else {
              // Last attempt failed, try upsert instead
              try {
                print("🔄 Final attempt using upsert...");
                await _supabase
                    .from('users')
                    .upsert({
                  'id': user.id,
                ...
                print("✅ User record created via upsert");
                userRecordCreated = true;
              } catch (upsertError) {
        ...
        if (userRecordCreated) {
          try {
            final verifyUser = await _supabase
                .from('users')
                .select('id, email')
                .eq('id', user.id)
                .maybeSingle();
            ...
            if (verifyUser != null) {
              print("✅ User record verified in database");
            } else {
              print("⚠️ User record not found after creation - this might cause issues");
            }
          } catch (e) {
            print("⚠️ Could not verify user record: $e");
          }
        }
