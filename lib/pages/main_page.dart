  ...
  Future<void> _loadProfileIcon() async {
    try {
      ...
      if (user != null) {
        ...
        print("Attempting to load profile icon for user: ${user.id}");
        print("Found icon path: $iconPath");
      }

      // Fallback to generic key if needed
      if (iconPath == null) {
        iconPath = prefs.getString('profile_icon_path');
        print("Fallback icon path: $iconPath");
      }

      if (iconPath != null && mounted) {
        setState(() {
          _profileIconPath = iconPath!;
          print("Set profile icon path to: $_profileIconPath");
        });
      }
    } catch (e) {
