  ...
  class _MainPageUIState extends State<MainPageUI> {
    ...
    void _showElementGuides() async {
      // First guide: Journey Card
      if (_journeyKey.currentContext != null) {
        final journeyBox = _journeyKey.currentContext!.findRenderObject() as RenderBox;
        final journeyPosition = journeyBox.localToGlobal(Offset.zero);

        await FocusedElementGuide.showIfNeeded(
          context,
          targetPosition: journeyPosition,
          targetSize: journeyBox.size,
      ...
      // Wait before showing the next guide
      await Future.delayed(const Duration(seconds: 1));

      // Second guide: Community Card
      if (_communityCardKey.currentContext != null) {
        final communityBox = _communityCardKey.currentContext!.findRenderObject() as RenderBox;
        final communityPosition = communityBox.localToGlobal(Offset.zero);

        await FocusedElementGuide.showIfNeeded(
          context,
          targetPosition: communityPosition,
          targetSize: communityBox.size,
