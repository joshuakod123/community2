  ...
  Future<void> _checkForNewNotifications() async {
    try {
      ...
      try {
        ...
        // Use jsonb data query instead of user_id column
        final response = await _supabase
            .from('notifications')
            .select()
            .eq('data->user_id', user.id) // Query using the "data" JSONB field
        ...
        // Try alternate query format if the data->user_id approach fails
        try {
          // Alternate approach - get all notifications and filter locally
          final response = await _supabase
              .from('notifications')
              .select()
              .gt('created_at', lastChecked)
          ...
          print('Error with alternate notification query: $innerError');
        }
      }
