...
class _NotificationSettingsPageState extends State<NotificationSettingsPage> {
  ...
  bool _communityEnabled = true; // Added for community notifications
  ...
  Future<void> _loadNotificationPreferences() async {
    final announcementsEnabled = await _notificationService
        .getNotificationPreference('announcements');
    final eventsEnabled = await _notificationService.getNotificationPreference(
        'events');
    final remindersEnabled = await _notificationService
        .getNotificationPreference('reminders');
    final communityEnabled = await _notificationService
        .getNotificationPreference('community');

    setState(() {
      _announcementsEnabled = announcementsEnabled;
      _eventsEnabled = eventsEnabled;
      _remindersEnabled = remindersEnabled;
      _communityEnabled = communityEnabled;
      _isLoading = false;
    });
  }
      ...
      body: _isLoading
        ...
        child: Column(
          ...
          children: [
            ...
            const Divider(color: Colors.grey),
            // New community notifications toggle
            _buildNotificationToggle(
              title: "Community",
              // You can replace with AppLocalizations when available
              subtitle: "Get notified when someone likes or comments on your posts",
              value: _communityEnabled,
              onChanged: (value) {
                setState(() {
                  _communityEnabled = value;
                });
                _toggleNotificationType('community', value);
              },
            ),
            const Divider(color: Colors.grey),
    ...
    try {
      ...
      if (mounted) {
        NotificationDisplayService.showPopupNotification(
          context,
          title: AppLocalizations.of(context).translate('notification_sent'),
          isSuccess: true,
        );
      }
    } catch (e) {
